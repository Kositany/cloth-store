      ...
      <main className="pt-20">
        ...
        <div className="container mx-auto px-4 py-4">
          <nav className="text-sm text-gray-400">
            ...
            <span>Shop</span>
            <span className="mx-2">/</span>
            <span className="text-neon-green">{product.name}</span>
          </nav>
        </div>
            ...
            <div className="space-y-4">
              <div className="relative aspect-square bg-gray-900 rounded-lg overflow-hidden">
                ...
                <div className="absolute top-4 left-4 space-y-2">
                  {product.isNew && (
                    <span className="bg-neon-green text-black px-3 py-1 rounded-full text-xs font-semibold">
                      NEW
                    </span>
                  )}
                  {product.isOnSale && (
                    <span className="bg-neon-pink text-white px-3 py-1 rounded-full text-xs font-semibold">
                      SALE
                    </span>
                  )}
                </div>
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex items-center space-x-4 mb-6">
                  ...
                  {product.isOnSale && (
                    <span className="bg-neon-pink text-white px-3 py-1 rounded-full text-sm font-semibold">
              ...
              <div className="space-y-4">
                ...
                <div className="flex space-x-4">
                  <motion.button
                    ...
                    className="flex-1 bg-neon-green text-black py-4 px-8 rounded-lg font-semibold hover:bg-neon-green/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
              ...
              <div className="grid grid-cols-3 gap-4 pt-6 border-t border-gray-800">
                <div className="text-center">
                  <ShieldCheckIcon className="w-8 h-8 text-neon-green mx-auto mb-2" />
                  <p className="text-sm text-gray-400">Premium Quality</p>
                </div>
                <div className="text-center">
                  <TruckIcon className="w-8 h-8 text-neon-blue mx-auto mb-2" />
                  <p className="text-sm text-gray-400">Free Shipping</p>
                </div>
                <div className="text-center">
                  <ArrowPathIcon className="w-8 h-8 text-neon-pink mx-auto mb-2" />
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {product.care.map((instruction, index) => (
                      <li key={index} className="flex items-center space-x-3">
                        <CheckIcon className="w-5 h-5 text-neon-blue flex-shrink-0" />
                      ...
                      <div key={review.id} className="border-b border-gray-800 pb-6">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center space-x-4">
                            <div className="flex items-center">
                              {[...Array(5)].map((_, i) => (
                                <StarSolidIcon
                                  key={i}
                                  className={`w-4 h-4 ${
                                    i < review.rating ? 'text-neon-green' : 'text-gray-600'
                                  }`}
                                />
                              ))}
                            </div>
                            ...
                            {review.verified && (
                              <span className="bg-neon-green text-black px-2 py-1 rounded text-xs font-semibold">
                                Verified
                              </span>
                            )}
                          </div>
                ...
                {relatedProducts.map((relatedProduct) => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-neon-green font-bold">${relatedProduct.price}</p>
                    </div>
                  </motion.div>
                ))}
              </div>
